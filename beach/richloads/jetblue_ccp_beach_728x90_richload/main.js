...
function animate() {
  ...
  .staggerFrom(['#copy4', '#copy5', '#copy6'], 1, { y: '+=200', ease: "expo.out" }, 0.1, 'start+=7')
    .to(['#endframe-right'], 0.5, { left: 0, ease: "expo.out" }, '+=3')
    .to(['#endframe_headline', '#terms'], 0.5, { autoAlpha: 1 }, '-=0.5')

    tl.to(['#sand, #man'], 7, { 
      transform: "translate3d(50px, 10px, 0) scale(1.05)", 
      ease: Power1.easeInOut 
    }, 0)
    // tl.to(['#sand', '#man'], 3, { transform: "translate3d(190px, 10px, 1px) scale(1.05)", ease: Power1.easeInOut }, 2)
    // tl.to(['#sky'], 3, { transform: "translate3d(50px, 15px, 1px)", ease: Power1.easeInOut }, 2)
    tl.to(['#sea'], 7, { 
      transform: "translate3d(0px, 10px, 0) scale(1.15)", 
      ease: Power1.easeInOut 
    }, 0)
    tl.to(['#sky'], 7, { 
      transform: "translate3d(10px, 0px, 0) scale(1.3)", 
      ease: Power1.easeInOut 
    }, 0)
}
