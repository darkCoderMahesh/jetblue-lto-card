...
function animate() {
  ...
  .to(['#logo'],0.5, { autoAlpha:0 },'+=3')  
  .to(['#endframe-right'], 0.5, { left: 0, ease: "expo.out" }, '>')
    .to(['#endframe_headline', '#terms'], 0.5, { autoAlpha: 1 }, '>')
    .to(['#logoWhite'],0.5, { autoAlpha:1 },'<')
    tl.to(['#man'], 7, { 
      transform: "translate3d(50px, 0, 0) scale(1.25)",
